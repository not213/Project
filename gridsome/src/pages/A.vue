...
<Layout>
  ...
  <b-nav tabs fill>
    ...
    <b-container class="bv-example-row">
        ...
        <div>
          <b-button block variant="primary" to="/table-a">Table / ตาราง</b-button>
        </div>
      <b-card-group columns>
        <b-card
          ...
          :title="edges.node.animal_name"
          :img-src="apiUrl+edges.node.image.url"
          img-alt="Image"
          img-top
        >
          <b-card-text>{{edges.node.animal_usable}} </b-card-text>
           <g-link class="post-tags__link" v-for="edges in edges.node.animal_types" :key="edges.id" :to="'Animal/'+edges.type">
              <b-button>#
                {{ edges.type }}
              </b-button>
            </g-link>
            <div>
              ...
              <b-modal v-model="modalShow">
                <b-img :img-src="apiUrl+edges.node.image.url"></b-img>
                <b-list-group>
    ...
    <div>
        <b-card class="text-center"
          ...
          style="max-width: 100%;"
        >
        <g-link class="post-tags__link" v-for="edges in $static.allStrapiAnimalType.edges" :key="edges.type" :to="'Animal/'+edges.node.type">
...
query Group($page: Int) {
  Strapi: allStrapiAnimal (perPage: 6, page: $page, sortBy: "id", order:ASC) @paginate {
    totalCount
        pageInfo {
          totalPages
          currentPage
...
export default {
  ...
  mounted() {
    this.Animals = this.$page.Strapi.edges
  ...
  computed: {
    ...
    searchResults () {
      ...
      for (let i=0; i<len; i++) {
        if (this.Animals[i].node.animal_name.includes(searchTerm)) {
          temp.push(this.Animals[i])
        }
      }
      return temp
