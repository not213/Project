...
<Layout>
  ...
  <b-nav tabs fill>
    ...
    <b-container class="bv-example-row">
        ...
        <div>
          <b-button to="/tableEconomy" block variant="primary">Table / ตาราง</b-button>
        </div>
      <b-card-group columns>
        ...
        >
          <b-card-text>{{edges.node.economy_kind}} </b-card-text>
           <g-link class="post-tags__link" v-for="edges in edges.node.economy_types" :key="edges.id" :to="'plant/'+edges.type">
              <b-button>#
                {{ edges.type }}
              </b-button>
            </g-link>
    ...
    <div>
        <b-card class="text-center"
          ...
          style="max-width: 100%;"
        >
        <g-link class="post-tags__link" v-for="edges in $static.allStrapiEconomyType.edges" :key="edges.type" :to="'plant/'+edges.node.type">
...
query Group ($page: Int) {
  Strapi: allStrapiEconomies (perPage: 6, page: $page, sortBy: "id", order:ASC) @paginate  {
    pageInfo {
      totalPages
      currentPage
       }
...
export default {
  ...
  mounted() {
    this.economy = this.$page.Strapi.edges
  ...
  computed: {
    ...
    searchResults () {
      ...
      for (let i=0; i<len; i++) {
        if (this.economy[i].node.economy_name.includes(searchTerm)) {
          temp.push(this.economy[i])
        }
      }
      return temp
